  ...
  try {
    ...
    if (subLength > 0) {
      while (searchTries != subLength) {
        const startMonth =
          dateAux.getMonth() + 1 > 9
            ? dateAux.getMonth() + 1
            : "0" + (dateAux.getMonth() + 1);
...
export const erpCreateSubscription = async (
  ...
  const startDateNumber =
    startDate.getDate() > 9
      ? startDate.getDate() + 1
      : "0" + startDate.getDate();
  const startDateString = `${startDate.getFullYear()}-${startMonth}-${startDateNumber}`;
  ...
  const endDateString = `${endDate.getFullYear()}-${endMonth}-${endDateNumber}`;
  const data = {
    docstatus: 0,
    party_type: "Customer",
